...
let answer = word.normalize('NFD').replace(/[\u0300-\u036f]/g, "").toUpperCase().split("");
console.log(answer);
...
const enterLetter = () => {
  ...
  if (control < 5 && empty === 0) {
    ...
    for (let i = 0; i < addStylesRight.length; i++) {
      addStylesRight[i].style.backgroundColor = "#66ff66";
    }
  }
  ...
  for (let i = 0; i < addStyleContain.length; i++) {
    addStyleContain[i].style.backgroundColor = "#ffe066";
  }
  for (let i = 0; i < addStylesRight.length; i++) {
    addStylesRight[i].style.backgroundColor = "#66ff66";
  }
